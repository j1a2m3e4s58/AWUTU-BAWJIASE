...
const normalizeFirebaseProjectId = (projectId, authDomain) => {
  const cleanAuthProjectId = String(authDomain || '').replace(/\.firebaseapp\.com$/, '');
  const cleanProjectId = String(projectId || '').trim();

  if (!cleanProjectId) {
    return cleanAuthProjectId;
  }

  if (cleanProjectId.includes('.firebaseapp.com') && cleanAuthProjectId) {
    return cleanAuthProjectId;
  }
