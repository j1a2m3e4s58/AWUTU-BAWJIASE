  ...
  return (
    <div>
      ...
      <section className="relative overflow-hidden pt-[4.6rem] lg:pt-24">
        <div className="max-w-7xl mx-auto px-5 sm:px-6 lg:px-10 py-12 sm:py-16 lg:py-28">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
            ...
            >
              ...
              <h1 className="font-display text-[2rem] sm:text-4xl md:text-5xl lg:text-6xl font-semibold text-foreground leading-[1.08]">
                {king?.name || t('lateKing')}
              </h1>
              {king?.title && (
                <p className="mt-3 font-display text-lg italic text-primary sm:text-xl">{king.title}</p>
              )}
              ...
              {king?.biography && (
                <p className="mt-5 max-w-lg text-sm leading-7 text-muted-foreground sm:mt-6 sm:text-base sm:leading-relaxed line-clamp-6">
                  {king.biography}
                </p>
              )}
            </motion.div>
            ...
            >
              <div className="aspect-[3/4] max-w-md mx-auto rounded-sm overflow-hidden shadow-2xl">
                <SmartImage
                  ...
                  loading="eager"
                  fallbackLabel="Memorial image unavailable"
                />
              </div>
            </motion.div>
          </div>
      ...
      {(memorialSettings.obituaryText || memorialSettings.burialDetails || memorialSettings.thanksgivingDetails || memorialSettings.familyAcknowledgements || memorialSettings.serviceOrder) && (
        <section className="py-14 lg:py-24">
          <div className="max-w-5xl mx-auto px-5 sm:px-6 lg:px-10">
            <div className="surface-panel rounded-sm p-5 sm:p-6 lg:p-8">
              ...
              {memorialSettings.obituaryText && (
                <div className="mb-8">
                  <h2 className="font-display text-xl sm:text-2xl font-semibold mb-3">Obituary</h2>
                  <p className="text-sm leading-7 text-muted-foreground whitespace-pre-wrap sm:text-base sm:leading-8">{memorialSettings.obituaryText}</p>
                </div>
              )}
              <div className="grid gap-6 md:grid-cols-2">
                {memorialSettings.burialDetails && (
                  <div>
                    <h3 className="font-display text-lg sm:text-xl font-semibold mb-3">Burial Details</h3>
                    <p className="text-sm leading-7 text-muted-foreground whitespace-pre-wrap">{memorialSettings.burialDetails}</p>
                  </div>
                )}
                {memorialSettings.thanksgivingDetails && (
                  <div>
                    <h3 className="font-display text-lg sm:text-xl font-semibold mb-3">Thanksgiving Details</h3>
                    <p className="text-sm leading-7 text-muted-foreground whitespace-pre-wrap">{memorialSettings.thanksgivingDetails}</p>
                  </div>
                )}
                {memorialSettings.familyAcknowledgements && (
                  <div>
                    <h3 className="font-display text-lg sm:text-xl font-semibold mb-3">Family Acknowledgements</h3>
                    <p className="text-sm leading-7 text-muted-foreground whitespace-pre-wrap">{memorialSettings.familyAcknowledgements}</p>
                  </div>
                )}
                {memorialSettings.serviceOrder && (
                  <div>
                    <h3 className="font-display text-lg sm:text-xl font-semibold mb-3">Service Order</h3>
                    <p className="text-sm leading-7 text-muted-foreground whitespace-pre-wrap">{memorialSettings.serviceOrder}</p>
                  </div>
                )}
              </div>
              {memorialSettings.programmeFileUrl && (
                <div className="mt-8">
                  <a
                    ...
                    className="inline-flex w-full items-center justify-center gap-2 border border-primary/30 px-4 py-2 text-sm text-primary transition-colors hover:bg-primary/10 sm:w-auto"
                  >
      ...
      <section className="py-16 lg:py-32 bg-card border-y border-border/50">
        <div className="max-w-4xl mx-auto px-5 sm:px-6 lg:px-10">
          ...
          <form onSubmit={handleSubmit} className="max-w-lg mx-auto mb-14 sm:mb-16 space-y-4">
            <Input
          ...
          {tributes.length > 0 && (
            <div className="flex justify-center mb-6">
              <button
                ...
                className="flex w-full items-center justify-center gap-2 border border-primary/30 rounded-sm px-4 py-2 text-sm text-primary transition-colors hover:text-primary/80 sm:w-auto"
              >
                <Download className="w-4 h-4" />
